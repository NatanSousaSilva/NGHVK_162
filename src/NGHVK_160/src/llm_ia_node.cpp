...
class Gemini_Node : public rclcpp::Node { // Herda da classe Node do ROS2
public:
    	Gemini_Node() : Node("llm_ia_node") { // Construtor

        publisher_ = this->create_publisher<std_msgs::msg::String>("llm_resposta", 10);
        subscription_ = this->create_subscription<std_msgs::msg::String>(
            	"llm_pergunta",10,
            	std::bind(&GeminiNode::subscription_callback, this, std::placeholders::_1)
        );
        RCLCPP_INFO(this->get_logger(), "Nó GeminiNode inicializado com sucesso.");
    }

private:
    	std::string api_key_ = "AIzaSyA7i7T3OdpTfAJoeM6HZLIW2Btz4pfuXMw";
	std::string prompt;
	...
	void subscription_callback(const std_msgs::msg::String::SharedPtr msg) {
    	...
    	void query_gemini(std::string prompt) {
        	...
        	curl_easy_setopt(curl, CURLOPT_USERAGENT, "libcurl-gemini-client/1.0");
		...
		try {
            		...
            		if (json_response.contains("candidates") &&
                		...
                		text = json_response["candidates"][0]["content"]["parts"][0]["text"].get<std::string>();
            		}
            		...
            		if (!text.empty()) {
                		...
                		RCLCPP_WARN(this->get_logger(), "Resposta JSON sem campo 'text'. JSON bruto: %s", response.c_str());
            		}
